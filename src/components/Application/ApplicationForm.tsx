...
type Application = {
  ...
  phone: string;
  comment: string;
  connection: "phone" | "email" | "telegram";
};
...
export default function ApplicationForm() {
  ...
  const onSubmit = (data: Application) => {
    const sendEmail = async () => {
      await axios.post("/api/send-notification", { ...data });
    };
  ...
  return (
    <form
      ...
      className="flex flex-col gap-3 sm:gap-2 w-[50%] sm:w-full"
    >
      <Modal
        isOpen={isOpen}
        classNames={{
          wrapper: "bg-[#333]/20 z-[102]",
          base: "bg-[#222]/85 border-solid overflow-y-auto border-[#ffff]/5",
          backdrop: "backdrop-blur-[12px] z-[101]",
        }}
        placement="center"
      ...
      <Select
        ...
        label="Как с вами лучше связаться?"
        isRequired
      >
      ...
      <div>
        <Button type="submit" size="lg" className="sm:w-full">
          оставить заявку
        </Button>
      </div>
    </form>
